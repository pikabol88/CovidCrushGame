...
public class Board : MonoBehaviour {
    ...
    void Start() {
        ...
        helpManager.ShowHelpPanel(level);
    }
    ...
    private void CheckToMakeBombs() {
        ...
        if (findMatches.currentMatches.Count > 3) {
            ...
            if (typeOfMatch.type == 1) {
                ...
                switch (findMatches.CheckBombs(typeOfMatch)) {                   
                    case 1:
    ...
    private void SwitchPieces(int column, int row, Vector2 direction) {
        if (!concreteTiles[column + (int)direction.x, row + (int)direction.y] && !lockTiles[column + (int)direction.x, row + (int)direction.y] && !slimeTiles[column + (int)direction.x, row + (int)direction.y]) {
    ...
    private bool CheckForMatches(int i, int j) {
        if (allDots[i, j] != null) {
            //Make sure that one and two to the right are in the
            //board
            if (i < width - 2) {
            ...
            if (i > 0 && i < width - 1) {
                if (allDots[i + 1, j] != null && allDots[i -1 , j] != null) {
                    if (allDots[i + 1, j].tag == allDots[i, j].tag
                       && allDots[i -1, j].tag == allDots[i, j].tag) {
                        return true;
                    }
                }
            }
            ...
            if (j > 0 && j < height - 1) {
                ...
                if (allDots[i, j + 1] != null && allDots[i, j - 1] != null) {
                    if (allDots[i, j + 1].tag == allDots[i, j].tag
                        && allDots[i, j - 1].tag == allDots[i, j].tag) {
                        return true;
                    }
                }
            }
