...
public class Dot : MonoBehaviour
{
    ...
    public float swipeResist = 1f; //минимальный свайп
    ...
    void Start() { 
        ...
        shineDelay = Random.Range(6f, 12f);

        //тк единственный объект данного типа
    ...
    private void Move()
    {
        if (Mathf.Abs(targetX - transform.position.x) > 0.1f || Mathf.Abs(targetY - transform.position.y) > 0.1f)
        {
    ...
    public IEnumerator CheckMoveCo() {
        ...
        //вернуть на место, если нет соответсвий
        if (otherDot != null) {
    ...
    //Помогает определять куда был совершен свайп
    void CalculateAngle() {
